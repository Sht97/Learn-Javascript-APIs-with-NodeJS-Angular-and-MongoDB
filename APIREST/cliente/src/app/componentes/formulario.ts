...
export class FormularioComponente implements OnInit{
  ngOnInit(){
  setTimeout(()=>{
    var pf = {
    ...
    var mf = {
          ...
          switch (input.target.id) {
            case "nombre":
              if (pf.valor.length < 2 || pf.valor.length > 6) {
                document.querySelector("[for=" + input.target.id + "] .error").innerHTML = '*Error al ingresar los datos: ' + input.target.placeholder + '';
      ...
      validarFormulario: function() {
        ...
        if (!pf.validarUsuario || !pf.validarPassword || !pf.validarEmail) {
          document.querySelector("#labelEnviar").innerHTML = '�*Tiene errores en los datos que ha ingresado, favor revisar de nuevo!';
          return false;
        } else if (!pf.validarTerminos) {
          document.querySelector("#labelEnviar").innerHTML = '�*Favor acepte t�rminos y condiciones!';
          return false;
        } else {
          return true;
        }
